...
#define RotByte(a) (((a >> 8) & 0x00FFFFFF) | ((a << 24) & 0xFF000000))

#define RotByteL(a) (((a >> 24) & 0x000000FF) | ((a << 8) & 0xFFFFFF00))

#define swap(a, b) {unsigned long t; t = a; a = b; b = t;}

// mult 2 elements using gf2_8_poly as a reduction
__inline unsigned char GF2_8_mult(unsigned char a, unsigned char b)
...
bool CheckRcon(bool create)
	...
	for (i = 1; i < sizeof(Rcon)/sizeof(Rcon[0])-1; i++)
		{
		if (create == true)
			Rcon[i] = Ri;
		else if (Rcon[i] != Ri)
...
void AES__KeyExpansion(const unsigned char * key)
	{
	int i;
	unsigned long temp, * Wb = (unsigned long*)(W); // todo not portable - Endian problems
...
void AES__SetParameters(int keylength, int blocklength)
	{
...
void AES__StartEncryption(const unsigned char * key)
	{
	AES__KeyExpansion(key);
	} // StartEncryption

void AES__EncryptBlock(const unsigned char * datain1, unsigned char * dataout1)
	...
	unsigned long * r_ptr = (unsigned long*)(W);
	unsigned long * dest  = state;
	unsigned long * src   = state; 
	const unsigned long * datain = (const unsigned long*)(datain1);
	unsigned long * dataout = (unsigned long*)(dataout1);
...
void AES__Encrypt(const unsigned char * datain, unsigned char * dataout, unsigned long numBlocks, BlockMode mode)
	{
	...
	switch (mode)
		...
		case ECB :
			while (numBlocks)
				{
				AES__EncryptBlock(datain,dataout);
				datain   += blocksize;
				dataout  += blocksize;
				--numBlocks;
				}
		...
		case CBC :
			...
			while (numBlocks)
				...
				for (pos = 0; pos < blocksize; ++pos)
					buffer[pos] ^= *datain++;
				AES__EncryptBlock(buffer,dataout);
...
void AES__StartDecryption(const unsigned char * key)
	{
	int pos, col;
	unsigned long * WL;
	...
	for (col = Nb; col < (Nr)*Nb; col++) // do all but first and last round
		{
		a0 = W_ptr[4*col+0];
		a1 = W_ptr[4*col+1];
		a2 = W_ptr[4*col+2];
	...
	WL = (unsigned long*)(W);
	for (pos = 0; pos < Nr/2; pos++)
		for (col = 0; col < Nb; col++)
			swap(WL[col+pos*Nb],WL[col+(Nr-pos)*Nb]);
	} // StartDecryption

void AES__DecryptBlock(const unsigned char * datain1, unsigned char * dataout1)
	{
	unsigned long state[8*2]; // 2 buffers
	unsigned long * r_ptr = (unsigned long*)(W);
	unsigned long * dest  = state;
...
void AES__Decrypt(const unsigned char * datain, unsigned char * dataout, unsigned long numBlocks, BlockMode mode)
	{
	unsigned int blocksize = Nb*4;
	...
	switch (mode)
		...
		case ECB :
			while (numBlocks)
				{
				AES__DecryptBlock(datain,dataout);
				datain   += blocksize;
				dataout  += blocksize;
				--numBlocks;
				}
		...
		case CBC :
			...
			unsigned int pos;
			memset(buffer,0,sizeof(buffer)); // clear out - todo - allow setting the Initialization Vector - needed for security
			AES__DecryptBlock(datain,dataout); // do first block
			for (pos = 0; pos < blocksize; ++pos)
				*dataout++ ^= buffer[pos];
			datain += blocksize;
			numBlocks--;
